...
###################### Load Data  ############################   
    X_train_set, y_train_set, X_test, y_test = k_fold_data_load(numFolds = 2)    
#>>>>>>>>>>>>>>>>>>>>> Load Data END   #<<<<<<<<<<<<<<<<<<<<
    ...
    for param in param_set:
        param_count+=1
        print('Currently, '+ str(param_count/len(param_set)*100) +'%' +' of parameter set are tested')
        ############################
        algorithm = param.algorithm 
        ############################               
        if algorithm is WhichAlgorithm.DCT:
            acc_tmp = 0
            Tree_max_depth = param.Tree_max_depth
            n_feats = param.n_feats
            n_threshold = param.n_threshold
            for X_train_, y_train_ in X_train_set, y_train_set:                
            ...
            if acc_result >= dct_prev_best_acc:
                ...
                param_set_.dct_best_param = param
            ...
            if acc_result >= rft_prev_best_acc:
                ...
                param_set_.rft_best_param = param
                param_save(param_set_,param_file_name)                
            ...
            if acc_result >= svm_prev_best_acc:
                ...
                param_set_.svm_best_param = param
                param_save(param_set_,param_file_name)                
            ...
            n_random_sample = param.n_random_sample
            ...
            if acc_result >= knn_prev_best_acc:
                ...
                param_set_.knn_best_param = param
                param_save(param_set_,param_file_name)                
